...
따라서 GC 등 추가적인 메모리 관리가 필요로 해지며, 자체 메모리 풀이 존재하며,
인스턴스 라이프사이클을 추적/관리 합니다.

다음과 같은 기능을 제공합니다:

1. 참조카운팅 기반 메모리 관리 (unique_ptr, shared_ptr 대비 개선)
2. meta 기반으로 한 구체 타입 바인딩
3. 자체 메모리 풀
4. 범용 바인더 클래스

<b>주의</b>: 아직 GC를 제공하진 않기에, 특별한 케이스에서는 메모리 누수가 발생합니다.
...
@ref by::binder "binder" 클래스는 범용 바인딩 클래스로 @ref by::instance "instance" 클래스를 상속한 클래스로부터 생성된 모든 객체를
바인딩할 수 있습니다.
...
binder는 크게 @ref by::tstr "tstr"과 @ref by::tweak "tweak" 2개를 제공합니다.
tstr은 강한참조를 갖는 바인더를, tweak는 약한 참조를 갖는 바인더 입니다.
...
```
@style: language-cpp verified
void me::rel(binder& me) { // me가 tstr인지 tweak인지 상관없다. 어떤 T인지도 상관없다.
    WHEN(!me.isBind()) .ret();

    life* l = me._getBindTag();
    if(l) l->_onStrong(-1);

    instance* inst = me.get(); // 물론 T를 모르니 instance로 받아야 한다.
}
```

@ref by::binder "binder" 는 abstract class 이므로 binder 객체 생성이 불가능합니다. @ref by::tstr "tstr" 이나 @ref by::tweak "tweak" 로 이미 생성된 바인더들을 범용적인
로직을 작성할때만 의의를 갖습니다.
...
@ref memlite 의 메모리 관리는 여러 클래스로 구성되어 있습니다. 각 클래스는 하나의 역할을 담당하며, 하위
계층의 클래스부터 이해하는 것이 전체 구조를 파악하는데 도움이 됩니다.

메모리 풀은 구조는 크게 2개의 가지로 분류되는데,
- pool - chunks - chunk로 이어지는 메모리 관리 컴포넌트
- watcher - life - bindTag 로 이어지는 라이프 사이클 관리 컴포넌트
...
메모리 관리 컴포넌트의 pool, chunks, chunk 각 클래스 역할이 뭔지를 파악하는게 코드 이해에 아주 중요합니다.
chunks는 여러개의 chunk를 관리하고, pool은 여러개의 chunks를 관리합니다.

### chunk - 최소 할당 단위

@ref by::chunk "chunk" 클래스는 @ref memlite 에서 유일하게 직접적으로 메모리를 실제로 할당 가능한 최소 단위 클래스입니다.
모든 메모리 관리는 @ref by::chunk "chunk" 들을 엮어서 수행합니다.
...
watcher와 life 클래스가 서로 연계하면서 객체의 라이프사이클을 추적, 관리합니다.
여기에는 `id` 라는 개체 식별값을 어떻게 정의하고 부여하는지가 매우 중요합니다.
...
<b>serial은 프로세스 실행 도중 instance 객체의 생성횟수</b>

@ref by::pool "pool" 과 @ref by::chunk "chunk" 를 봤다면 알겠지만, 자체 메모리 풀을 사용하기 때문에 메모리가 해제 될때는 소멸자만
...
관리하는 @ref by::watcher "watcher" 를 가지고 있습니다. Facade 패턴을 사용하여 복잡한 메모리 관리 서브 패키지에 대한 단순화된 인터페이스를 제공합니다.
