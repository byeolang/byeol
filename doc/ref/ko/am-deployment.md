...
## exec 대신 spawn을 쓰는 이유

Unix 계열에는 현재 프로세스를 다른 프로그램으로 바꿔치기하는 exec가 있습니다. launcher처럼
"넘기고 빠지는" 역할에는 얼핏 더 어울려 보이죠. 그런데 Windows에는 이에 대응하는 기능이 없습니다.

플랫폼마다 다른 방식을 쓰면 프로세스 트리 모양이 달라지고, 그 차이가 자식 정리나 세션 추적 같은
곳에서 미묘한 버그로 되돌아옵니다. 그래서 <b>양쪽 플랫폼 모두 spawn 후 대기하는 방식으로
통일</b>했습니다. 어느 운영체제에서든 프로세스 구조가 동일하게 유지되죠.

프로세스를 하나 더 띄우는 비용이 걱정될 수 있지만, 인터프리터가 시작해서 소스를 파싱하는 비용에
비하면 무시할 수 있는 수준입니다. 빌드 대기를 줄이자는 byeol의 방향과도 충돌하지 않습니다.

---

## 플랫폼 종속 코드의 격리

프로세스를 생성하고, 표준 입출력을 물려주고, 자식을 정리하는 일은 운영체제마다 방식이 완전히
다릅니다. Unix에서는 fork나 posix_spawn을 쓰고 파일 디스크립터가 자연스럽게 상속되지만,
Windows에서는 `CreateProcess`에 상속 옵션을 켜고 표준 핸들을 따로 지정해야 합니다.

이런 차이는 전부 @ref indep 모듈 안에 갇힙니다. launcher를 비롯한 상위 코드는 추상화된
인터페이스만 보며, `#ifdef`를 직접 쓰지 않습니다. 조건부 컴파일은 오직 indep 내부의 플랫폼별
구현 파일에만 존재하죠.

경계에서 플랫폼 고유 타입을 노출하지 않는 것도 중요한 규칙입니다. Windows의 `HANDLE`이나
Unix의 `pid_t`가 상위 코드에 보이면 그 순간 플랫폼 독립성이 깨집니다. 대신 프로젝트가 정의한
추상 타입만 상위로 전달됩니다.

이 원칙은 @ref ae-architecture-overview 에서 설명하는 계층 구조가 launcher에도 똑같이 적용된
결과입니다. indep이 여러 저장소에서 공유되는 이유이기도 하죠.
