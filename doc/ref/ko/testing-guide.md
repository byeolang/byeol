...
Unit Test는 개별 클래스나 함수의 기능을 독립적으로 검증하는 테스트입니다. 다른 모듈에 의존하지 않고 작은 단위의 로직을 세밀하게 테스트할 때 사용합니다. 예를 들어 `args` 클래스의 생성자가 올바르게 동작하는지, `add` 메서드가 예상대로 요소를 추가하는지 등을 확인합니다.
...
Unit Test 파일의 기본 구조는 매우 단순합니다. 먼저 `test/byeolTest.hpp` 헤더를 포함하고, `by`와 `std` 네임스페이스를 사용 선언합니다. 그런 다음 `byeolTest`를 상속받는 테스트 클래스를 정의합니다. 이 클래스는 별도의 멤버를 추가하지 않고 빈 구조체로 두는 것이 일반적입니다.
...
다음은 `args` 클래스의 다양한 생성자를 테스트하는 예제입니다. 각 테스트는 특정 생성자 하나에 집중하며, 해당 생성자의 동작을 철저히 검증합니다.
...
Integration Test는 Unit Test와 달리 익명 네임스페이스를 사용하는 것이 일반적입니다. 이는 테스트 클래스가 해당 파일에만 보이도록 제한하여 링커 충돌을 방지하기 위함입니다.
...
`parse()` 메서드에는 byeol 소스 코드를 전달하는데, 이때 C++11의 Raw String Literal을 사용합니다. `R"SRC( ... )SRC"` 형식을 사용하면 이스케이프 문자 없이 코드를 작성할 수 있고, 들여쓰기도 그대로 유지됩니다. 시작과 끝의 구분자(여기서는 `SRC`)는 반드시 일치해야 합니다.

### 파싱 결과 검증

`shouldParsed(true)`는 파싱이 성공해야 함을 의미하고, `shouldParsed(false)`는 파싱이 실패해야 함을 의미합니다. 문법 오류가 있는 코드를 테스트할 때는 `shouldParsed(false)`를 사용합니다. 이 메서드는 실제 결과와 기대값이 일치하면 `true`를 반환하므로, `if`문과 함께 사용하여 파싱 성공 후 추가 검증을 수행할 수 있습니다.
...
오류 케이스를 테스트할 때는 반드시 `negative()` 메서드를 호출해야 합니다. Negative TC에서는 파서나 인터프리터가 의도적으로 실패하는데, 시스템의 기본 동작이 실패 시 로그를 출력하는 것이기 때문에 `negative()`를 호출하지 않으면 마치 오동작한 것처럼 로그창이 지저분해집니다. `negative()`를 호출하면 이러한 예상된 실패에 대한 로그 출력을 억제하여 테스트 결과를 깔끔하게 유지할 수 있습니다. Antipattern 테스트에서는 거의 모든 테스트 케이스에 `negative()`가 포함됩니다.
...
다음은 함수 정의와 함수 호출을 구분하는 기능을 테스트하는 예제입니다.
...
Integration Test에서 파싱 결과를 가져올 때는 항상 `OR_ASSERT` 매크로를 사용해야 합니다. 이를 사용하지 않으면 null 포인터를 역참조하여 세그멘테이션 폴트가 발생할 수 있습니다.
...
void SetUp() override {
    arr::_cache.clear();
    byeolIntegTest::SetUp();  // 부모 호출 필수
}

void TearDown() override {
    // 정리 작업
    byeolIntegTest::TearDown();  // 부모 호출 필수
}
```
...
심각한 버그나 세그멘테이션 폴트가 발생하면 디버거를 사용해야 합니다. Linux에서는 GDB를, macOS에서는 LLDB를 사용합니다.
...
정상 케이스뿐만 아니라 오류 케이스도 반드시 테스트해야 합니다. 시스템이 잘못된 입력에 대해 적절히 에러를 발생시키는지 확인하는 것도 중요합니다. Antipattern 테스트는 이러한 음수 테스트를 체계적으로 관리하는 방법입니다.

### 의미 있는 테스트 데이터

매직 넘버 대신 의미 있는 값을 사용합니다. 테스트 코드를 읽는 사람이 왜 그 값을 사용했는지 이해할 수 있어야 합니다. 필요하다면 상수를 정의하여 사용하는 것도 좋습니다.

### 리소스 정리

테스트가 생성한 리소스는 반드시 정리해야 합니다. 메모리 누수를 방지하기 위해 `TearDown` 메서드에서 리소스를 해제합니다. byeol 프로젝트는 메모리 관리를 중요하게 다루므로, 테스트에서도 이를 철저히 지켜야 합니다.

### 커스텀 실패 메시지

필요한 경우 ASSERT 매크로에 커스텀 메시지를 추가할 수 있습니다. `<<` 연산자를 사용하여 실패 시 출력될 메시지를 지정합니다.

```cpp
ASSERT_EQ(a.len(), 3) << "Array length should be 3 after adding 3 elements";
```

이 메시지는 테스트가 실패했을 때만 출력되며, 디버깅에 유용한 정보를 제공합니다.

---

## 빌드 및 실행

테스트를 작성한 후에는 빌드하고 실행해야 합니다. 새로운 파일을 추가했다면 전체 빌드를 수행해야 하고, 기존 파일만 수정했다면 증분 빌드로 충분합니다.

### 전체 빌드

새로운 테스트 파일을 추가했거나 프로젝트 구조가 변경되었을 때는 전체 빌드를 실행합니다.

```bash
./build/builder.py dbg
```

이 과정은 시간이 오래 걸리므로 필요할 때만 수행합니다.

### 증분 빌드

기존 파일의 함수나 클래스만 수정했을 때는 증분 빌드를 사용합니다.
...
이 가이드를 따라 테스트를 작성하면 byeol 프로젝트의 코드 품질과 안정성을 유지할 수 있습니다. 테스트는 코드가 올바르게 동작함을 보장하고, 리팩토링 시 안전망 역할을 하며, 새로운 기여자가 코드를 이해하는 데 도움을 줍니다.
