...
이외의 언급하지 않은 문법은 markdown 문법을 그대로 사용하면 되죠. 다만 문제가 생길 경우, 항상 Doxygen이 이를 처리한다는 점을 염두에 두어야 합니다.
...
가이드 문서에서 가장 중요한 요소 중 하나는 예시입니다. 아무리 자세하게 설명해도 예시가 없으면 불완전한 설명이에요. 예시에는 positive 예시와 negative 예시를 함께 보여주면 좋습니다.
...
이를 해결하기 위해서 코드 블록 안에 JSON을 추가하면 됩니다. JSON은 `style`, `shown`, `code` 3가지 필드를 사용해야 합니다. `style`는 annotation들이고, `shown`은 코드 블록에서 보여져야 할 코드입니다. `code`는 인터프리터로 실제로 실행하게 될 전체 소스입니다.
