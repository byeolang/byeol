...
이것이 가능한 이유는 `with A`가 **"A라는 대상을 현재의 문맥(Context)에 포함시킨다"**는 하나의 일관된 원리로 동작하기 때문입니다.

*   **상속**: 부모 객체를 내 문맥에 포함시키면? 부모의 멤버들이 내 것처럼 보이게 됩니다. 이것이 곧 상속입니다.
*   **Import**: 다른 파일(모듈)을 내 파일의 문맥에 포함시키면? 그 파일의 함수들을 바로 쓸 수 있습니다. 이것이 곧 Import입니다.
*   **Static**: 어떤 타입 자체를 문맥에 포함시키면? 인스턴스를 만들지 않고도 그 타입의 함수를 쓸 수 있습니다. 이것이 곧 Static 접근입니다.
...
*   **생산(구현)**은 정밀한 텍스트 코딩으로 합니다.
*   **파악과 디버깅**은 전체를 볼 수 있는 시각화 도구(VPL)처럼 합니다.
