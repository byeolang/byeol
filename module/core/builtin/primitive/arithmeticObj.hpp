...
namespace by {
    ...
    public:
        ...
        tstr<me> add(const me* it) const BY_SIDE_FUNC(add);

        using super::sub;
        tstr<me> sub(const me& rhs) const;
        tstr<me> sub(const me* it) const BY_SIDE_FUNC(sub);

        tstr<me> mul(const me& rhs) const;
        tstr<me> mul(const me* it) const BY_SIDE_FUNC(mul);

        tstr<me> div(const me& rhs) const;
        tstr<me> div(const me* it) const BY_SIDE_FUNC(div);

        tstr<me> mod(const me& rhs) const;
        tstr<me> mod(const me* it) const BY_SIDE_FUNC(mod);

        tstr<me> bitwiseAnd(const me& rhs) const;
        tstr<me> bitwiseAnd(const me* it) const BY_SIDE_FUNC(bitwiseAnd);
        tstr<me> bitwiseXor(const me& rhs) const;
        tstr<me> bitwiseXor(const me* it) const BY_SIDE_FUNC(bitwiseXor);
        tstr<me> bitwiseOr(const me& rhs) const;
        tstr<me> bitwiseOr(const me* it) const BY_SIDE_FUNC(bitwiseOr);
        tstr<me> lshift(const me& rhs) const;
        tstr<me> lshift(const me* it) const BY_SIDE_FUNC(lshift);
        tstr<me> rshift(const me& rhs) const;
        tstr<me> rshift(const me* it) const BY_SIDE_FUNC(rshift);
        virtual tstr<me> bitwiseNot() const = 0;

        nbool eq(const me& rhs) const;
        nbool eq(const me* it) const BY_SIDE_FUNC(eq);
        nbool ne(const me& rhs) const;
        nbool ne(const me* it) const BY_SIDE_FUNC(ne);
        nbool gt(const me& rhs) const;
        nbool gt(const me* it) const BY_SIDE_FUNC(gt);
        nbool lt(const me& rhs) const;
        nbool lt(const me* it) const BY_SIDE_FUNC(lt);
        nbool ge(const me& rhs) const;
        nbool ge(const me* it) const BY_SIDE_FUNC(ge);
        nbool le(const me& rhs) const;
        nbool le(const me* it) const BY_SIDE_FUNC(le);
        nbool logicalAnd(const me& rhs) const;
        nbool logicalAnd(const me* it) const BY_SIDE_FUNC(logicalAnd);
