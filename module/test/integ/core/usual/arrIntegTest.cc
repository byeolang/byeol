...
namespace {
    ...
    void benchMarkArr(int cnt) {
        ...
        BY_I("[benchMarkArr]: arr took total %d ms for adding(%dms) & removing(%dms) of %d elems.",
            (nint64) (totalElapsed / chrono::milliseconds(1)), (nint64) (addingElapsed / chrono::milliseconds(1)),
            (nint64) (removingElapsed / chrono::milliseconds(1)), sz);
    }
} // namespace

TEST_F(arrIntegTest, instantiateTest) { arr arr1; }

TEST_F(arrIntegTest, shouldNotCanAddLocalObject) {
    tarr<myNode> arr1;
...
TEST_F(arrIntegTest, simpleAddDelTest) {
    tarr<myNode> arr1;
    ASSERT_EQ(arr1.len(), 0);
...
TEST_F(arrIntegTest, addDel10Elems) {
    tarr<myNode> arr1;
    const int cnt = 10;
    for(int n = 0; n < cnt; n++)
...
TEST_F(arrIntegTest, benchMarkArrTest) {
    benchMarkArr(100);
    benchMarkArr(1000);
    benchMarkArr(10000);
}

TEST_F(arrIntegTest, testIter) {
    arr arr1;
...
TEST_F(arrIntegTest, testContainableAPI) {
    //  initial state:
...
TEST_F(arrIntegTest, testcloneDeep) {
    narr arr1;
    arr1.add(new myNode(1));
    arr1.add(new myNode(2));
    ASSERT_EQ(arr1.len(), 2);
...
TEST_F(arrIntegTest, testRangeBasedForLoop) {
    narr arr1;
...
TEST_F(arrIntegTest, genericMarshalingTest) {
    nInt int1;
    arr arr1(int1);
...
TEST_F(arrIntegTest, testSimpleBridgedFuncs) {
    tarr<nInt> arr1;
    node& it = arr1;
...
TEST_F(arrIntegTest, testSimpleBridgedFuncs2) {
    tarr<nInt> arr1;
    node& it = arr1;
...
TEST_F(arrIntegTest, testIteratorBridgedFunc) {
    tarr<nInt> arr1;
    node& it = arr1;
...
TEST_F(arrIntegTest, newInstanceSharesFuncs) {
    arr a(*new nInt(0));
...
TEST_F(arrIntegTest, testBasicDefSyntax) {
    make()
        .parse(R"SRC(
...
TEST_F(arrIntegTest, testImplicitlyDefSyntax) {
    make()
        .parse(R"SRC(
        print(msg str) void: 1
...
TEST_F(arrIntegTest, testImplicitlyefSyntax2) {
    make()
        .parse(R"SRC(
        sum(arr flt[]) flt
            ret arr[0] + arr[2]
...
TEST_F(arrIntegTest, arrDeductionFailNegative2) {
    make()
        .negative()
        .parse(R"SRC(
        def obj
...
TEST_F(arrIntegTest, arrDeductionFailNegative3) {
    make()
        .negative()
        .parse(R"SRC(
        def obj
...
TEST_F(arrIntegTest, arrDeductionFailNegative4) {
    make()
        .negative()
        .parse(R"SRC(
        def obj
...
TEST_F(arrIntegTest, addFromEmptyArray) {
    make()
        .parse(R"SRC(
        def person
            value str
...
TEST_F(arrIntegTest, testDeepCopy) {
    make()
        .parse(R"SRC(
        main() int
            org := {1, 2}
...
TEST_F(arrIntegTest, testShallowCopy) {
    make()
        .parse(R"SRC(
        main() int
            org := {1, 2}
...
TEST_F(arrIntegTest, test2DArray) {
    make()
        .parse(R"SRC(
        main() int
            arr := {{0, 1}, {1, 2}}
...
TEST_F(arrIntegTest, test2DArray2) {
    make()
        .parse(R"SRC(
        main() int
            arr := {{0, 1}, {1, 2}}
...
TEST_F(arrIntegTest, test3DArray3) {
    make()
        .parse(R"SRC(
        print(msg str) void: 1
...
TEST_F(arrIntegTest, testCallCtor) {
    make()
        .parse(R"SRC(
        main() int
            a := {1, 2, 3}
...
TEST_F(arrIntegTest, addDifferentElemTypeNegative) {
    make()
        .negative()
        .parse(R"SRC(
        main() void
...
TEST_F(arrIntegTest, namelessArr) {
    make()
        .parse(R"SRC(
        main() int
            {"hello", "world"}.len()
...
TEST_F(arrIntegTest, accessElementOnce) {
    make()
        .parse(R"SRC(
        def p
            cnt := 0
...
TEST_F(arrIntegTest, setElemConversion) {
    make()
        .parse(R"SRC(
        main() int
            arr := {"a", "c"}
...
TEST_F(arrIntegTest, setElemConversion1) {
    make()
        .parse(R"SRC(
        foo() int
            0
...
TEST_F(arrIntegTest, setElemAddAssignConversion) {
    make()
        .parse(R"SRC(
        foo() int
            0
...
TEST_F(arrIntegTest, setElemAddAssignConversion2) {
    make()
        .parse(R"SRC(
        foo() int
            1
...
TEST_F(arrIntegTest, setFieldOfElemAfterGetIt) {
    make()
        .parse(R"SRC(
        def A
            age int
...
TEST_F(arrIntegTest, setFieldOfElemAfterGetIt1) {
    make()
        .parse(R"SRC(
        def A
            age int
...
TEST_F(arrIntegTest, defPropWithCustomTypeShouldWork) {
    make()
        .parse(R"SRC(
        def Person
            age int
