...
TEST_F(mgdObjIntegTest, testAccessInCompleteObjectNegative) {
    make()
        .negative()
        .parse(R"SRC(
        def A
...
TEST_F(mgdObjIntegTest, testAccessInCompleteObjectNegative2) {
    make()
        .negative()
        .parse(R"SRC(
        def a
...
TEST_F(mgdObjIntegTest, clonedObjDoesntCloneSharesDeeply) {
    make()
        .parse(R"SRC(
        def b
            grade := 3.5
